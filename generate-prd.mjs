...
const children = [
  ...
  bullet("يعرض جميع حالات الهامش والنواقص المرتبطة بالملف"),
  bullet("كل حالة تحتوي: الفاتورة المصدر، الصنف، الكميات الناقصة، المبلغ، الحالة"),
  bullet("فلتر الحالة: تحت الفحص / تم الحل"),

  ...emptyLine(),
  sectionHeading("طرق إضافة حالة هامش:", 3),
  ...infoBox(
    "الطريقة ١ — تلقائية (عند استلام الفاتورة)",
    [
      "عند فتح نموذج استلام فاتورة وإدخال كمية مستلمة أقل من المطلوب في أي سطر",
      "النظام يحسب الفرق تلقائياً ويعرض تنبيه: 'سيتم إنشاء X قيد هامش تلقائياً للنواقص'",
      "عند تأكيد الاستلام تُنشأ حالة هامش لكل سطر به نقص بالكميات والقيمة المحسوبة",
    ],
    COLOR.LIGHT_BG,
    COLOR.ACCENT
  ),
  ...infoBox(
    "الطريقة ٢ — يدوية (من صفحة الهوامش)",
    [
      "المسار: التجارة المحلية ← الهوامش  →  زر 'حالة هامش جديدة'",
      "حقل الطرف: اختيار الملف (تاجر أو عميل) من قائمة بحث",
      "حقل الفاتورة: اختيار الفاتورة المرتبطة بالطرف المحدد",
      "حقل الوصف: شرح نصي للمشكلة أو سبب الهامش (إلزامي)",
      "الحالة الافتراضية عند الإنشاء: 'تحت الفحص'",
    ],
    COLOR.GREEN_BG,
    "16A34A"
  ),

  sectionHeading("خيارات تسوية حالة الهامش:", 3),
  makeTable(
    ["الوصف", "يطلب مبلغ؟", "خيار التسوية"],
    [
      ["قبول الهامش وتسوية القيمة مع الطرف", "نعم", "هامش مقبول"],
      ["استبدال المنتج بآخر مماثل", "لا", "استبدال"],
      ["خصم قيمة المنتج من حساب الطرف", "نعم", "خصم القيمة"],
      ["شطب المنتج كتالف بدون مطالبة مالية", "لا", "تالف"],
    ],
    [4500, 2000, 2500]
  ),
  ...emptyLine(),
  bullet("عند التسوية يمكن تسجيل: عدد القطع والكراتين المُعادة + ملاحظة القرار"),
  bullet("بعد التسوية تتغير حالة الحالة من 'تحت الفحص' إلى 'تمت التسوية' (أخضر)"),
